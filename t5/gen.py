...
def pro(token_list, tokenizer):
    ...
    string = "".join(string)
    # print(string)
    # string = tokenizer.decode(token_list, skip_special_tokens=False, spaces_between_special_tokens=False, clean_up_tokenization_spaces=False)
    # print(string)
...
if generation:
    ...
    # exit()
    file_out = "./result/%s_%s_%s.txt"%(model_name_path.replace("/", "_").replace(".", ""), task_name, mode)
    ...
    with open(file_out, "w") as fout:
        ...
        with torch.no_grad():
            while ed < len(ipt):
                ...
                if mode == "0":
                    decoder_input = "<s>0<eop>" # 生成关键词时是<eod> 生成最终评论时是<eop>
                elif mode == "1":
                    decoder_input = "<s>1<eop>"
                else:
                    decoder_input = "<s>"
                decoder_input_ids = torch.tensor([tokenizer.convert_tokens_to_ids(tokenizer.tokenize(decoder_input)) for _ in range(ed-st)], dtype=torch.long).to(device)
                # gen = model.generate(input_ids, decoder_input_ids=decoder_input_ids, do_sample=True, max_length=512, top_p=0.9, decoder_start_token_id=1)

                gen = model.generate(input_ids, decoder_input_ids=decoder_input_ids, do_sample=True, max_length=512, top_p=0.9, decoder_start_token_id=1, 
                    min_length=5, num_beams=1, length_penalty=1., no_repeat_ngram_size=0)# num_beams=4 do_sample=True, top_p=0.9)#, temperature=0.7) decoder_start_token_id=0
                # try:
                ...
                for ip, op in zip(ipt[st:ed], gen):
                    if debug:
                        print(st, ed)
                        print(ip)
                        print(op)
